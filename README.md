# error-codes-and-messages-for-firebase-authentication-API
Full list of all auth/ errors.

## Here is some for auth/*:
The keys are being added to auth/ and the values are the default error messages.
## usage example

```js

	// Sign up GOOGLE
	const authenticateUsserWithGoogle = async () => {
		try {
			...
			if (error.code === 'auth/popup-closed-by-user') {
				toast('You cancelled the signup action');
			}
		}
	};
```
